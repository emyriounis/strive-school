  ...
  const showEditExperience = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/profile/${process.env.REACT_APP_ME_ID}/experiences/${selectedExperience}`
      );
    ...
    <>
      <Col xs="11" className="pl-3 mb-2">
        ...
        <p className="text-muted mb-1">
          ...
          {endDate ? format(parseISO(endDate), "MMM. yyyy") : "Present"} &#8226;{" "}
          {endDate
            ? differenceInCalendarMonths(parseISO(endDate), parseISO(startDate))
            : differenceInCalendarMonths(new Date(), parseISO(startDate))}{" "}
          months
        </p>
