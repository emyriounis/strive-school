...
const SingleSearchResult = ({ data, type }) => {
  ...
  return (
    <Row className="single-list-item my-1 py-3">
      <Col xs="1">
        <img
          src={type === "profile" ? data.image : data.user.image}
          className="profile-image-posts"
          alt=""
        />
      </Col>
      <Col xs="9">
        <Link to={`/profile/${data._id}`} className="text-dark">
          <h6 className="mb-0">
            {type === "profile" ? data.name : data.user.name}{" "}
            {type === "profile" ? data.surname : data.user.surname} &#8226;{" "}
            <span className="text-muted font-weight-normal">Following</span>
          </h6>
        </Link>
        <p className="reduced-text mb-0">
          {type === "profile" ? data.bio : data.user.bio}
        </p>
        {type === "profile" ? (
      ...
      <Col xs="2">
        <button
          ...
          className="button main-btn-outline mt-3 ml-2"
        >
          {type === "profile" ? "Connect" : "View Post"}
        </button>
      </Col>
      {type === "posts" && (
