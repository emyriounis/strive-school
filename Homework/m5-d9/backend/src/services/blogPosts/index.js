...
import { sendBlogConfirmationEmail } from "../../lib/email-tools.js";
import { getPDFReadableStream, createTempPDF } from "../../lib/pdf-tools.js";
    ...
    if (!errorsList.isEmpty()) {
      ...
      const tempPDF = await createTempPDF(
        getPDFReadableStream({
          ...newBlogPost,
          // cover: "public/img/authorAvatar/1dguni1w01kwmcbj1h.jpeg",
        })
      );
      sendBlogConfirmationEmail({
        recipientAddress: "el.myrioun@gmail.com",
        pdfBlogPost: tempPDF,
      });
    }
  } catch (error) {
...
blogPostsRouter.get("/:id/comments", async (req, res, next) => {
  try {
    ...
    // console.log(blogPostComments.comments);
